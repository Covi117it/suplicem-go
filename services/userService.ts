...
export const createUserAccountByAdmin = async (data: any) => {
  if (data instanceof FormData) {
    return await safeRequest(() => protectedApi.post("/users", data));
  }
  ...
  return await safeRequest(() => protectedApi.post("/users", formData));
};
